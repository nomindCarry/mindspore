...
void UpdateOutputDeviceInfo(const std::vector<device::DeviceAddressPtr> &device_addressess,
  ...
  for (size_t i = 0; i < output_num; i++) {
    ...
    auto shape = kernel->Shape()->cast<abstract::ShapePtr>();
    if (shape && !shape->IsDynamic()) {
      device_addressess[i]->set_host_shape(shape->shape());
    }
...
void UpdateInputInCompileInfo(const OpCompilerInfoPtr &op_compiler_info, const std::vector<TensorPtr> &input_tensors,
  ...
  auto inputs = op_compiler_info->graph_->inputs();
...
void UpdateGraphInputAbstract(const KernelGraphPtr &graph, const session::BackendOpRunInfoPtr &op_run_info,
  ...
  for (size_t i = 0; i < input_size; ++i) {
    ...
    auto abstract = std::make_shared<abstract::AbstractTensor>(type_of_tensor, input_tensors[i]->shape());
    input_nodes[i]->set_abstract(abstract);
    auto op_runtime_info = input_nodes[i]->user_data<runtime::OpRuntimeInfo>();
    if (op_runtime_info != nullptr) {
      op_runtime_info->Resize(input_nodes[i]);
    }
  }
}
...
void LaunchKernelsDynamicNew(const pynative::OpCompilerInfoPtr &op_compiler_info,
  ...
  MS_LOG(DEBUG) << "Start";
  UpdateGraphInputAbstract(op_compiler_info->graph_, op_run_info, op_compiler_info->device_context_);
...
void RunSingleOpDynamic(const session::BackendOpRunInfoPtr &op_run_info, const OpCompilerInfoPtr &op_compiler_info,
  ...
  ReleaseKernelResource(op_compiler_info->graph_);
}
}  // namespace mindspore::runtime
