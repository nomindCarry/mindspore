...
void MindRTBackend::RunOpImplDynamic(bool single_op_cache_hit, const OpCompilerInfoPtr &op_compiler_info,
  ...
  if (!async_exec_disabled) {
    ...
    // create output tensor
    UpdateOutputDynamicAsync(op_compiler_info, outputs, device_address_list, op_run_info->base_op_run_info.abstract);
...
void MindRTBackend::UpdateOutputDynamic(const OpCompilerInfoPtr &op_compiler_info, VectorRef *const outputs,
  ...
  for (size_t i = 0; i < outputs_size; ++i) {
    ...
    auto output_tensor = CreateOutputTensorDynamic(item_with_index.first, item_with_index.second, output_address);
    MS_EXCEPTION_IF_NULL(output_tensor);
...
void MindRTBackend::UpdateOutputDynamicAsync(const OpCompilerInfoPtr &op_compiler_info, VectorRef *const outputs,
                                             const vector<device::DeviceAddressPtr> &device_address_list,
  ...
  for (size_t i = 0; i < outputs_size; ++i) {
    ...
    MS_EXCEPTION_IF_NULL(output_address);
    auto output_tensor =
      CreateOutputTensorDynamic(item_with_index.first, item_with_index.second, output_address, out_abstract);
    MS_EXCEPTION_IF_NULL(output_tensor);
    output_tensor->set_lazy_callback([]() { runtime::OpExecutor::GetInstance().Wait(); });
    outputs->emplace_back(output_tensor);
  }
